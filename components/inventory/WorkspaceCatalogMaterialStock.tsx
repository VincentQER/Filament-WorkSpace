...
type Props = { brand: string; material: string };

export function WorkspaceCatalogMaterialStock({ brand, material }: Props) {
  const { items, authReady, updateItems, undo, undoStackLength } = useInventoryWorkspace();
  const [search, setSearch] = useState("");

  const catalogRows = useMemo(() => {
    const mk = normalizeKey(material);
    return colorsForBrandCatalog(brand)
      .filter((e) => normalizeKey(e.material) === mk)
  ...
  const rowQuantities = useMemo(() => {
    ...
    for (const entry of catalogRows) {
      const matches = items.filter((i) => itemMatchesBrandCatalogEntry(i, brand, entry));
      if (matches.length === 0) {
  ...
  function adjustRow(entry: CatalogColorJson, field: "spools" | "refills" | "openSpoolWeight", delta: number) {
    updateItems((prev) => {
      const matches = prev.filter((i) => itemMatchesBrandCatalogEntry(i, brand, entry));
      const rest = prev.filter((i) => !itemMatchesBrandCatalogEntry(i, brand, entry));
      let merged = matches.length ? mergeMatches(matches) : makeCatalogItem(entry, brand);
              ...
              {filteredCatalog.map((entry, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 font-mono text-xs text-violet-400/85">{entry.sku?.trim() || "—"}</td>
                    <td className="px-4 py-2.5">
