...
export default function WorkshopPage() {
  ...
  // Load the user's inventory to power "Matches your inventory" badges
  useEffect(() => {
    void (async () => {
  ...
  // Sort: relevant items first within each view
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        {/* Affiliate disclosure — legally required by FTC & Amazon ToS */}
        <p className="mt-3 rounded-xl border border-white/[0.06] bg-zinc-900/60 px-4 py-2.5 text-[11px] leading-relaxed text-zinc-500">
          <span className="font-semibold text-zinc-400">Disclosure:</span> Links on this page
          are Amazon affiliate links. If you purchase through them, we earn a small commission
          at no extra cost to you. We only recommend products we&apos;d actually use.
        </p>
      </div>

      {/* Personalisation banner — only if we found matches */}
      {relevantCount > 0 && (
        <div className="flex items-start gap-3 rounded-2xl border border-emerald-500/20 bg-emerald-950/30 px-4 py-3">
          ...
          <div>
            <p className="text-sm font-semibold text-emerald-300">
              Personalised for your inventory
            </p>
            <p className="mt-0.5 text-xs text-zinc-400">
              {relevantCount} product{relevantCount !== 1 ? "s" : ""} matched to the
              materials you already track. They&apos;re highlighted in green.
            </p>
          </div>
        </div>
      )}
